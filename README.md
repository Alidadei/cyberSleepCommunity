...
- **在线（主）**：CloudBase 静态托管，国内快、可绑域名，见 [docs/deploy-guide.md](docs/deploy-guide.md)
- **在线（兜底）**：GitHub Pages（Settings → Pages 开启 main 分支即可）
...
- [docs/deploy-guide.md](docs/deploy-guide.md) —— 域名方案：CloudBase 静态托管为主 + GitHub Pages 兜底
...
| P1 云端互通 | 试用中 | 数据后端 = Supabase 免费档（填两行配置即通）；域名 = CloudBase 主 + GitHub Pages 兜底 |
